...
def sales_new():
    ...
    if request.method == "POST":
        ...
        if request.form.get("apply_service_fee"):
            sale.service_fee_usd = round(subtotal * Sale.SERVICE_FEE_RATE, 2)
        sale.shipping_estimate_usd = float(request.form.get("shipping_estimate_usd") or 0)
...
def _send_quotation_email(quote, to_email):
    """Renders the shared document email template for this quotation and sends it.
    Returns True if a send was actually attempted (SMTP configured), False otherwise —
    the caller decides what to tell the user based on that."""
    html = render_template(
        "email/document_email.html",
        title="QUOTATION", doc_label="Quote No", doc_number=quote.quote_number,
        date_str=quote.date.strftime("%d %B %Y"),
        extra_header_lines=[f"Valid Until: {quote.valid_until.strftime('%d %B %Y')}"] if quote.valid_until else [],
        customer_name=quote.display_customer, items=quote.items, items_subtotal=quote.items_subtotal,
        service_fee=quote.service_fee_usd, shipping=quote.shipping_estimate_usd, extra_rows=[],
        total_label="Total", total=quote.total, notes=quote.notes, recipient_email=to_email,
    )
    text_fallback = (f"Quotation {quote.quote_number}\nTotal: ${quote.total:.2f}\n\n"
                      f"Log in to your Duoshao account to view the full details, or contact us with any questions.")
    sent = send_html_email(to_email, f"Duoshao Trading — Quotation {quote.quote_number}", html, text_fallback)
    if sent and quote.status == "draft":
        quote.status = "sent"
    return sent


def _send_receipt_email(receipt, to_email):
    sale = receipt.sale
    html = render_template(
        "email/document_email.html",
        title="RECEIPT", doc_label="Receipt No", doc_number=receipt.receipt_number,
        date_str=receipt.issued_date.strftime("%d %B %Y"),
        extra_header_lines=[f"Method: {receipt.method.replace('_',' ').title()}"],
        customer_name=sale.display_customer, items=sale.items, items_subtotal=sale.items_subtotal,
        service_fee=sale.service_fee_usd, shipping=sale.shipping_estimate_usd,
        extra_rows=[
            ("Sale Reference", f"#{sale.id}"),
            ("Sale Total", f"${sale.total:.2f}"),
            ("Total Received to Date", f"${sale.amount_received:.2f}"),
            ("Balance Remaining", f"${sale.balance_due:.2f}"),
        ],
        total_label="This Receipt", total=receipt.amount, notes=None, recipient_email=to_email,
    )
    text_fallback = f"Receipt {receipt.receipt_number}\nAmount received: ${receipt.amount:.2f}\n\nThank you for your payment."
    return send_html_email(to_email, f"Duoshao Trading — Receipt {receipt.receipt_number}", html, text_fallback)


@admin_bp.route("/quotations/<int:quote_id>/email", methods=["POST"])
@login_required
def quotation_email(quote_id):
    quote = Quotation.query.get_or_404(quote_id)
    to_email = request.form.get("to_email", "").strip() or (quote.customer.email if quote.customer else None)
    if not to_email:
        flash("No email address on file for this client — type one in below to send it manually.", "error")
        return redirect(url_for("admin.quotation_detail", quote_id=quote.id))
    sent = _send_quotation_email(quote, to_email)
    db.session.commit()
    if sent:
        flash(f"Quotation emailed to {to_email}.", "success")
    else:
        flash("Email wasn't sent — SMTP isn't configured yet (see email_utils.py for setup steps).", "error")
    return redirect(url_for("admin.quotation_detail", quote_id=quote.id))


@admin_bp.route("/receipts/<int:receipt_id>/email", methods=["POST"])
@login_required
def receipt_email(receipt_id):
    receipt = Receipt.query.get_or_404(receipt_id)
    to_email = request.form.get("to_email", "").strip() or (receipt.sale.customer.email if receipt.sale.customer else None)
    if not to_email:
        flash("No email address on file for this client — type one in below to send it manually.", "error")
        return redirect(url_for("admin.receipt_detail", receipt_id=receipt.id))
    sent = _send_receipt_email(receipt, to_email)
    if sent:
...
def quotation_new():
    ...
    if request.method == "POST":
        ...
        if quote.customer and quote.customer.email:
            sent = _send_quotation_email(quote, quote.customer.email)
            db.session.commit()  # persist status='sent' if the send flipped it from 'draft'
            msg += (f" Emailed to {quote.customer.email}." if sent else
                    " Email not sent — SMTP isn't configured yet; you can print it or send it manually below.")
        else:
            msg += " No email on file for this client — print it or enter an email manually on the quotation page."
        flash(msg, "success")
        return redirect(url_for("admin.quotation_detail", quote_id=quote.id))
